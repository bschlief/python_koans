...
class AboutGenerators(Koan):
    ...
    def test_generators_can_take_coroutines(self):
        ...
        next(generator)
        # The reason next has to be called is to store the result of the message
        # into result for the next iteration.  The generator will then return
    ...
    def test_send_none_is_equivalent_to_next(self):
        ...
        # BMS: This means that .send(None) passes in nothing, so we get the else clause above
